...
def check_database_connection(cfg: dict) -> tuple[bool, str]:
    ...
    driver = driver.lower() if driver else ""
    if driver in ("mysql", "postgresql", "postgres", "pg", "mssql", "sqlserver", "sql_server"):
        ...
        if not (db.get("service_name", "") or "").strip():
            return False, "❌ 数据库配置错误: Oracle 需配置 service_name（如 DB_SERVICE_NAME=ORCL）"
    ...
    try:
        if driver in ("mysql",):
            ...
            # 达梦：database/DB_NAME 可选；仅 host:port 即可连默认实例
            try:
                import dmPython
                p = int(port) if port else 5236
                if database:
                    dsn = f"{host}:{p}/{database}"
                else:
                    dsn = f"{host}:{p}"
                conn = dmPython.connect(user, password, dsn)
                conn.close()
                tail = f"/{database}" if database else ""
                return True, f"✅ 数据库连接正常 (dm://{host}:{p}{tail})"
            except ImportError:
