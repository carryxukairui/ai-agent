...
def validate_database_config(config: Dict[str, Any]) -> List[str]:
    ...
    按驱动区分「库名」类字段：MySQL/PostgreSQL/SQL Server 需要 database；
    Oracle 使用 service_name；达梦 database（或 DB_NAME）为可选，用于实例名/模式，可省略。
    ...
    driver = (db.get("driver", "mysql") or "mysql").lower()
    errors.extend(
    ...
    if driver in ("mysql", "postgresql", "postgres", "pg", "mssql", "sqlserver", "sql_server"):
        if not str(db.get("database", "")).strip():
            errors.append(
                "database.database: 此数据库类型需配置 database（环境变量 DB_NAME）"
            )
    elif driver in ("oracle", "cx_oracle"):
        if not str(db.get("service_name", "")).strip():
            errors.append(
                "database.service_name: Oracle 需配置 service_name（环境变量 DB_SERVICE_NAME）"
            )
    elif driver in ("dm", "dmdb", "dameng"):
        pass
    else:
        if not str(db.get("database", "")).strip():
            errors.append("database.database: 需配置 database")
